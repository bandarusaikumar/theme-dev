...
$page_column_class = emall_page_layout_columns_class($theme_options['ts_blog_details_layout'], $theme_options['ts_blog_details_left_sidebar'], $theme_options['ts_blog_details_right_sidebar']);

$show_breadcrumb = apply_filters('emall_show_breadcrumb_on_single_post', true);
$show_page_title = is_singular('post') ? true : false;

emall_breadcrumbs_title($show_breadcrumb, $show_page_title, is_singular('post') ? __('Blog', 'emall') : '');
	...
	<div id="main-content">		
		<article class="single single-post <?php echo esc_attr($post_format); ?> <?php echo !$show_blog_thumbnail?'no-featured-image':''; ?>">
			...
			<header>
				...
				<?php if( $theme_options['ts_blog_details_date'] || $theme_options['ts_blog_details_author'] || $theme_options['ts_blog_details_comment'] ) : ?>
					<div class="entry-meta-middle">
					
						<?php if( $theme_options['ts_blog_details_author'] ): ?>
						<span class="vcard author">
							<span><?php esc_html_e('By', 'emall'); ?></span>
						...
						<span class="date-time">
							<span><?php esc_html_e('On', 'emall'); ?></span>
						...
						<span class="comment-count">
							<?php
							$comment_count = emall_get_post_comment_count();
							echo sprintf( _n('%d Comment', '%d Comments', $comment_count, 'emall'), $comment_count );
							?>
						</span>
						<?php endif; ?>
			...
			<div class="entry-author">
				...
				<div class="author-info">
					<div class="author-meta">
						...
						<span class="role"><?php echo emall_get_user_role( get_the_author_meta('ID') ); ?></span>
					</div>
			...
			<div class="meta-bottom-2">
				<!-- Next Prev Blog -->
				<div class="single-navigation prev"><?php previous_post_link('%link', '<span>'. esc_html__('Previous Post', 'emall') .'</span> <span>%title</span>'); ?></div>
				<!-- Next Prev Blog -->
				<div class="single-navigation next"><?php next_post_link('%link', '<span>'. esc_html__('Next Post', 'emall') .'</span> <span>%title</span>'); ?></div>
			</div>
			<?php endif; ?>
			...
			?>
			<div class="single-related-wrapper"><?php get_template_part('templates/related-posts'); ?></div>
			<?php } ?>
