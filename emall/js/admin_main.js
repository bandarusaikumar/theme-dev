...
jQuery(function($){
	...
	$('.ts-gallery-box .add-image').on('click', function(event){
        ...
        var _states = [new wp.media.controller.Library({
            filterable: 'uploaded',
            title: emall_admin_texts.select_images,
            multiple: true,
            priority:  20
        })];
        ...
        file_frame = wp.media.frames.file_frame = wp.media({
            states: _states,
            button: {
                text: emall_admin_texts.use_images
            }
        });
	...
	/* Copy table */
	$('.ts-meta-box-field.table .copy-table').on('click', function(e){
		e.preventDefault();
		if( typeof localStorage != 'undefined' ){
			var field = $(this).parent().siblings('.field');
			localStorage.setItem( field.find('table').prop('id'), field.find('.table-value').val() );
			$(this).val( $(this).data('clicked') );
		}
	});
	
	/* Paste table */
	$('.ts-meta-box-field.table .paste-table').on('click', function(e){
		e.preventDefault();
		if( typeof localStorage != 'undefined' ){
			var table = $(this).parent().siblings('.field').find('table');
			
			var data = localStorage.getItem( table.prop('id') );
			if( !data ){
				alert( emall_admin_texts.paste_table_data_error );
				return;
			}
			...
			if( Array.isArray( data ) ){
				var row_add_column = table.find('.add-col').length ? 1 : 0;
				
				data.forEach(function(row_data, row_index){
					if( Array.isArray( row_data ) ){
						if( row_index == 0 && row_add_column == 1 ){ /* add col */
							var number_column = row_data.length;
							while( table.find('tr:first td').length <= number_column ){
								table.find('tr:first .add-col:last').trigger('click');
							}
						}
						
						if( !table.find('tr').eq(row_index + row_add_column).length ){ /* add row */
							table.find('tr:last .add-row').trigger('click');
						}
						
						var table_row = table.find('tr').eq(row_index + row_add_column);
						row_data.forEach(function( val, col_index ){
							if( table_row.find('input').eq(col_index).length ){
								table_row.find('input').eq(col_index).val( val );
							}
						});
					}
				});
...
jQuery(function($){
	...
	if( add_sidebar_form.length > 0 ){
		...
		$('#ts-add-sidebar').on('click', function(e){
			...
			if( sidebar_name != '' ){
				...
				var data = {
					action: 'emall_add_custom_sidebar'
	...
	if( $('.sidebar-ts-custom-sidebar').length > 0 ){
		...
		$('.sidebar-ts-custom-sidebar .delete-sidebar').on('click', function(){
			...
			var sidebar_nonce = $('#ts_custom_sidebar_nonce').val();
			var ok = confirm( emall_admin_texts.delete_sidebar_confirm );
			if( ok ){
				widget_block.hide();
				var data = {
					action: 'emall_delete_custom_sidebar'
					,sidebar_name: sidebar_name
					,sidebar_nonce: sidebar_nonce
				};
				...
				$.ajax({
					...
					,success : function(response){
						if( response != '' ){
							...
							widget_block.show();
							alert( emall_admin_texts.delete_sidebar_failed );
						}
					}
				});
			}
...
jQuery(function($){
	...
	$( document ).on( 'click', '.ts-product-cat-upload-field .upload-button', function( event ) {
		...
		file_frame = wp.media.frames.downloadable_file = wp.media({
			title: emall_admin_texts.choose_an_image,
			button: {
				text: emall_admin_texts.use_image
			},
			multiple: false
		});
...
jQuery(function($){
	...
	var edit_button_class = 'ts-post-edit-button';
	var view_button_class = 'ts-post-view-button';
	
	$(document).on('change initial', 'select.ts-post-select', function(){
		update_post_edit_view_buttons( $(this) );
	});
	
	$('select.ts-post-select').trigger('initial');
	...
	function update_post_edit_view_buttons( sel ){
		...
		if( !sel.siblings('.' + edit_button_class).length ){
			sel.after('<a class="' + edit_button_class + '" href="#" target="_blank" style="display: none">' + emall_admin_texts.edit_post_button_label + '</a>');
		}
		if( !sel.siblings('.' + view_button_class).length ){
			sel.after('<a class="' + view_button_class + '" href="#" target="_blank" style="display: none">' + emall_admin_texts.view_posts_button_label + '</a>');
		}
		
		if( val != 0 && val != '' ){
			sel.siblings('.' + edit_button_class).attr('href', emall_admin_texts.edit_post_url_pattern[post_type].replace('[post_id]', val)).show();
			sel.siblings('.' + view_button_class).hide();
		}
		else{
			sel.siblings('.' + view_button_class).attr('href', emall_admin_texts.view_posts_url_pattern.replace('[post_type]', post_type)).show();
